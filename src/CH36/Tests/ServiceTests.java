...
//		}
	...
	void bookServiceImpl_session() throws Exception {
		...
		service.bookRegistration(new BookDto(1122L,"이것이C언어다","오렌지미디어","121212"),3);
		for(String key : response.keySet()) {
			System.out.println(key + " : " + response.get(key));
		}
	}
	@Test
	void bookServiceImpl_update() throws Exception {
		BookService service = BookServiceImpl.getInstance();
		Map<String,Object> response= 
		service.bookUpdate(new BookDto(1122L,"이것이C언어다","오렌지미디어","121212"),3);
		for(String key : response.keySet()) {
			System.out.println(key + " : " + response.get(key));
		}
	}
